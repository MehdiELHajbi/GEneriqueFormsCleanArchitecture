    ...
    public class IfDatabaseExisteStep : IRule<Context>
    {
        ...
        public async Task<Context> Execute(Context ctx)
        {
            ...
            if (existe != null)
            {
                ...
                //ctx.OneOfCreateDataBaseResponse.ExceptionDataBaseAlreadyExistsResponse
                //                            = new ExceptionDataBaseAlreadyExistsResponse(this.nameDatabse);
                //throw new CustomException(CustomException.ExceptionType.ExisteDejaException, "valeur existe deja");
                //new ExceptionCustom(
                //                     OneOfResponseExceptionCreate.ExceptionType.ExceptionDataBaseAlreadyExists
                //                    , new ExceptionDataBaseAlreadyExistsResponse(this.nameDatabse)
                //                    );
                ThrowException(ctx);
            }
        ...
        {
            return await this.dataBaseRepository.GetFirstOrDefault(s => s.NameDataBase == this.nameDatabse);
        }
